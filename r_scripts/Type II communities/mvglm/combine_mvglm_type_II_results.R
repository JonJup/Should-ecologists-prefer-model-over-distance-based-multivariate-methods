...
setwd(dir = "~/01_Uni/My Papers/1909_Should ecologists prefer model- over algorithm-based multivariate methods/r_scripts/reviewer_comments/01_different_response_same_gradient/drsg_mvglm/")
...
for (i in seq_along(output.files)) {
   ...
   analysis.data[,2] = save.obj[[1]][["table"]][2:5,3]
   analysis.data[,3] = save.obj[[1]][["table"]][2:5,4]
   analysis.data[,4] = paste0("drsg", i)
   analysis.data[,5] = rep(as.numeric(save.obj[[2]], units = "secs"), times = 4)
   analysis.data[,6] = NA
   analysis.data[,7] = rep("mvglm", n = 4)
...
saveRDS(cca_combine, "../../03_analyse_results/mvglm/mvglm_drsg_results.RDS")
