...
safe_cqo = safely(cqo)
p.values = c()
print(paste("Finished Fitting Model for 2. Started p-values @", Sys.time()))
for (column in 31:34) {
      ...
      for (i in 1:length(x)) {
            coef[i,] <- summary(x[[i]][[1]])@post$Coef@C[column - 30,]
      }
      
      coef2 <- apply(abs(coef),1,sum)
      p.values[column - 30] <-  length(which(coef2 > sum(abs(summary(cqo.pr2)@post$Coef@C[column - 30,])))) / (length(coef2) + 1)
      
      print(paste("P Value for Varaible", column, "calculated @", Sys.time()))
...
print("Now we start with community 5")
...
# load data 
data = readRDS("02_variable_abundance_communities/vac5.RDS")
...
safe_cqo = safely(cqo)
p.values = c()
print(paste("Finished Fitting Model for 5. Started p-values @", Sys.time()))
for (column in 31:34) {
...
saveRDS(object = output, 
        file = "~/01_Uni/My Papers/1909_Should ecologists prefer model- over algorithm-based multivariate methods/r_scripts/reviewer_comments/02_variable_abundance_communities/vac_cqo/cqo_results/vac5_cqo_resuts.RDS")
