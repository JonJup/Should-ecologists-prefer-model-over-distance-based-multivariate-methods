...
#------------------#
    ...
    out <- sim.re.md(n.grad = 2, re.type = list(rep(1,9), rep(1,9)), parameter = para, grad = env)
    ...
    Simulation.output <- list("data" = cbind(out.samp,envn), "Response.Type" = "LL", "Samples" = sites^2)
    rm(out.grid, out.samp, sample, out, para,sites, nerr, env, envn, n, complement, make.noise, sim.re.md, take.sample)
