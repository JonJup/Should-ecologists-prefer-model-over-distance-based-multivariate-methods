...
\section{Introduction}
	...
	more species than sites (\textit{high dimensionality}, especially in DNA barcoding studies, \citep{cristescu2014barcoding}),  
...
% 	% 
% 	For instance, Minkowski distances (e.g. Manhattan and Euclidean) assume a constant variance across all mean values \cite{TerBraak1988}
% 	%
% 	whereas species abundances often show a quadratic mean-variance relationship \cite{routledge1991taylor, yamamura1999transformation}.
% 	%
...
% 	%
% 	The choice is complicated by the vast amount of available metrics \cite[see][]{Legendre2012}.
% 	%
...
% 	The model-based approach consists of explicitly specifying a statistical model of the process that generated the observed data \cite{Warton2015a}.
% 	%
...
% 	%
% 	While this approach is ubiquitous in univariate analyses \cite[e.g.][]{bolker2008ecological, zuur2010protocol}, it has long been uncommon in multivariate ecological analyses, largely due to the absence of suitable models \cite{anderson2001new}.
% 	%
...
% 	%
% 	Recent examples include Hierarchical Modeling of Species Communities \cite{Ovaskainen2017}, Generalized Joint 
% 	%
% 	Attribute Modelling \cite{Clark2017},  and multivariate Generalized Linear Models \cite[MvGLM,][]{Warton2012}.
...
% 	%
% 	Univariate GLMs are a flexible method and are strongly advocated for the analysis of count or occurrence data as they can handle different residual distributions and mean-variance relationships \cite{OHara2010, Warton2011, Szocs2015_b}.
% 	%
% 	Extending them to multi-species abundance data was thus a natural starting point for multivariate model-based analyses \cite{Warton2012}.
% 	%
% 	The univariate models are combined by summing their test statistics, which allows for inference on the whole community.
% 	%
% 	The use of MvGLM, facilitated by an easy-to-use implementation in R  \cite[in the \textit{mvabund} R-package, ][]{Wang2019}, has steadily increased within the ecological community. 
% 	%
...
% 	% CQO
% % 	Another model-based method based on GLMs is the Constrained Quadratic Ordination (CQO), which is the maximum likelihood solution to Constrained Gaussian Ordination \cite[CGO,][]{Yee2004}, in which species are expected to respond unimodally to latent gradients that are linear combinations of environmental variables \cite{gauch1974ordination}.
% % 	%
% % 	CQO uses an extension to GLM, the \textit{Vector Generalized Linear Model} \cite[VGLM,][]{yee2003reduced}. 
% % 	% CCA
...
% % 	%
% % 	CCA is one of the most widely used and cited multivariate statistical techniques in ecology \cite{Braak2014}.
% % 	% db-RDA
...
% % 	%
% % 	Distance-based RDA (dbRDA) represents a flexible extension to RDA \cite{Legendre1999, AndersonMati2003}. 
% %     %
...
% % 	%
% % 	dbRDA was highlighted by \citet{Szocs2015}, because the possibility to use asymmetrical distance metrics and avoid the \textit{double-zero problem } \cite{Legendre2012} makes them attractive for sparse data sets. 
% % 	%
...
% 		Species abundances were simulated as counts, a common abundance measure in ecology \cite{Warton2008}. 
% 		%
...
% 		%&		Responses				
% 		%------------------------------%
% 		Unimodal responses were simulated using the Gaussian response model \cite{GauchJr1972} expanded to multiple dimensions (Eqn. \ref{eq:GaussianResponseModel}).
% 		%
...
% % 		%
% % 		The \textit{p}-values of these are adjusted to multiple testing by controlling the family-wise type I error rate using a resampling-based version of the \textit{Holm's step-down multiple testing procedure} \cite{Westfall1993}. \\
% % 		%
% % 		%These adjusted \textit{p}-values tend to be very conservative. \\
% 		%
% 		We fit MvGLMs with Poisson, negative binomial and Gaussian residual distributions to each community and compared their Dunn-Smyth residual-plots \cite{dunn1996randomized} and Akaike's Information Criteria \cite[AIC, ][]{akaike1974new}.
% 		%
% 		The likelihood ratio test statistic was calculated for the best fitting model (least patterns in residuals and lowest AIC).
% 		%
% 	    To estimate \textit{p}-values, we used a residual permutation bootstrap with 1000 repetitions \cite{davidsonbootstrap}.
...
%         In its current implementation in the VGAM R-package \cite{VGAM19}, CQO does not provide \textit{p}-values \cite[but see][]{yee2010vglms}. 
%         %
...
% 	 CCA is the heuristic solution to Restricted Gaussian Regression \cite{Zuur2007}. 
% 	 %
...
% 	 %
% 	 Instead, \cite{TerBraak1986} proposed to approximate the results by CCA, which is valid as long as: all species have equal tolerances $t$ and maximal abundances $c$, their responses are unimodal and symmetrically bell-shaped and their optima $c$ are spread uniformly in the ordination space. 
...
% 	 %
% 	 Today, CCA is one of the most widely used and cited multivariate statistical methods in ecology \cite{Braak2014}.\\
% 		%
...
% 		%
% 		Hypothesis tests for environmental variables can be conducted using a pseudo-$F$ statistic with permuted residuals  \cite{Legendre2011} and the null hypotheses that the effect of the variable on the response is equal to zero after accounting for the effect of all other variables. 
% 	    %
...
%         %
% 		It is the constrained form of Principal Coordinates Analysis (PCoA) \cite{Legendre1999}, which will be shortly addressed here.
% 		%
% 		In PCoA, $\mathbf{Y}$ is transformed into a centered distance matrix $\Delta$.
% 		%
% 		The columns of the matrix $\mathbf{PC}$ are the eigenvectors of $\Delta$ scaled to a length that is equal to the square root of their eigenvalues \cite{gower1966some}.
% 		%
...
% 		%  	
% 	    We used the Bray-Curtis distance, which is the reciprocal of the Steinhaus coefficient  \cite{motyka1947zadaniach}, to calculate $\Delta$.
% 	    %
% 		Negative eigenvalues were corrected with Lingoes correction \cite{lingoes1971some}.
% 		%
...
%         %
%         To alleviate the problematic dichotomy of statistical significance \cite{Greenland2016}, we use five different significance levels $\alpha$ (0.01, 0.03, 0.05, 0.07 and 0.1).
% 		%
% 		This allows us to evaluate trends in classification strength over different thresholds. 
        

% 	\subsection{Software}
% 		All simulations and analyses were done in R 3.4.4 \cite{RCT2018}.
% 		%
% 		MvGLM were conducted with mvabund 3.13.1. \cite{Wang2019}, dbRDA and CCA with vegan 2.5-2 \cite{Oksanen2018} and CQO with VGAM 1.0-5 \cite{VGAM19}. 
% 		%
% 		%
...
% 	%
% 	A drawback of MvGLMs is the long run time due to resampling \cite{Wang2012}.
% 	%
% 	The resampling is necessary because it ensures that inferences are valid, even if species are intercorrelated. 
% 	%
% 	Resampling observations across independent sites (i.e. rows) accounts for their possible correlation \cite{anderson2001new}. 
%     %
...
%     %
%     Another option is shrinking the correlation matrix towards identity using ridge regularization \cite{warton2008penalized, Warton2011a}.
% 	%
...
% 	%  
% 	Having to choose a single metric can be avoided by using consensus RDA \cite{Blanchet2014}.
% 	% 
...
% 	%
% 	As CCA assumes unimodal gradients, which are more frequent than linear ones in nature \cite{Oksanen2002}, this was expected.  
% 	%
...
% 	%
% 	Newer approaches to CCA that can correct for zero inflation \cite{Zhang2012} or non-linear relationships between predictor and response variable \cite{Makarenkov2002} are available but not widely used. 
% 	%
...
% %& 		Simulated Data			
% %------------------------------% 
%     The realism of the simulated communities could be improved by using more complex response patterns like beta-functions \cite{austin1994determining}, which add asymmetries to bell-shaped curves.
% 	%
...
% 	%
% 	Simulations with induced endogeneity would be interesting as this phenomenon is underappreciated by ecologists \cite{armsworth2009contrasting, fox2015ecological}.
% 	%
% 	Observation and measurement are sources of errors in field data sets, and both can be represented in a model via binomial functions as in N-mixture models \cite{royle2004n}.
% 	%
%     This would be interesting to examine the effects of regression dilution \cite{frost2000correcting, McInerny2011}.
...
% 	as these two sets of predictors (biotic and abiotic) were shown to have little redundancy \cite{Meier2010}.
% 	%
% 	Some of the models also anticipate the growing challenges of Big Data for ecology \cite{Hampton2013}.
% 	%
% 	Generalized Linear Latent Variable Models, for example, include latent variables instead of random effects to capture residual correlation, which considerably reduces the size of the variance -- covariance matrix \cite{Warton2015,Niku2017}.  
% 	%
% 	In Hierarchical Modeling of Species Communities \cite{Ovaskainen2017} this approach is coupled with a fourth corner model \cite[including species traits, ][]{legendre1997relating} and phylogenetic relationships to create a flexible and comprehensive framework for community data analysis.
% 	%
...
%     %
% 	It is now essential that ways to infer ecological processes from the modeled patterns develop at a similar pace as these models, to avoid confusing statistical artifacts with genuine biological signals \cite{dormann2018biotic}.
% 	%
% 	If this succeeds, a move from algorithm-based towards model-based methods might entail one from the current implicit Gleassonian towards a modern form of Clementsian perspective \cite{Eliot2011}; from asking how do individual species change along environmental gradients towards asking how do communities change as a whole.   
...
%     % 
%     The method of choice for such cases are permutation-based tests, which produce pseudo \textit{p}-values \cite{Legendre2012}. 
%     %
