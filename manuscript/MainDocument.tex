...
%%% ----------------- %%%
    ...
    %%% ---------------- %%%
        ...
        %conclusions
          Our study shows that both model- and algorithm-based methods have their place in the ecologist's statistical toolbox. 
          % 
...
\section{Material and Methods}
	...
	%------------------------------%
        ...
        The simulation process is visualized in Figure \ref{fig:flowchart_simulation}.
        %
        More details on the parameterization of the models are provided in Table S 1 in the Appendix.
...
\section{Results}
		...
		We report the means and standard deviations of \textit{p}-values of MvGLM, CQO, CCA, and dbRDA for all explanatory variables (Table \ref{table:results1::mean-p}), \textit{p}-values for different response shapes and sample sizes are given in the Tables S 2 - 5 in the Appendix.
        ...
        \begin{table*}[ht]
            \centering
        	\caption{
        	    Mean \textit{p}-values $\pm$ standard deviations of the causal (\textit{env1} and \textit{env2}) and noise variables from multivariate Generalized Linear Models (MvGLM), Constrained Quadratic Ordination (CQO), Canonical Correspondence Analysis (CCA), and distance-based Redundancy Analysis (dbRDA) for all models. 
        	    }
            \begin{tabular}{@{}
	...
	%------------------------------%
        ...
        \begin{figure}[h]
            \centering
            \includegraphics[scale = 0.7]{figure3_falsenegativeandpositives}
            \caption{
            False Positive Rate and False Negative Rate of the four statistical methods Canonical Correspondence Analysis (CCA), Constrained Quadratic Ordination (CQO), distance-based Redundancy Analysis (db-RDA), and multivariate Generalized Linear Model (MvGLM).
            }
            \label{fig:FPNR}
        \end{figure}{}
	...
	%------------------------------%
		...
		%
		The FPR was relatively high, with 0.031 at $\alpha = 0.05$.  
		% 
        \textit{p}-values were relatively similar for all sample sizes (Table S 5). 
        %
...
%------------------------------%
	...
	%
	The FNR was also low and all Fale Negatives occured in communities with the smallest sample size.
	%
    ...
    %
	MvGLMs are the only method considered here that does not reduce the dimesionality of the data.
	%
...
%------------------------------%
    ...
    %
    These properties, together with the low FPR and FNR and the modest computation time make it an excellent method for multivariate analysis in ecology.
	%
